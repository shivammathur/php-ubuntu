...
add_packages apache2 automake file gcc g++ git jq make pkg-config python3-apt shtool libtool sudo systemd unzip
add_packages autoconf firebird-dev freetds-dev libacl1-dev libapparmor-dev libargon2-dev libaspell-dev libbrotli-dev libc-ares-dev libdb-dev libedit-dev libelf1t64 libgomp1 libgpgme-dev libicu-dev libkrb5-dev libldap-dev liblmdb-dev liblz4-dev libmaxminddb-dev libmemcached-dev libnghttp2-dev libonig-dev libpq-dev libqdbm-dev librabbitmq-dev libsodium-dev libsnmp-dev libsqlite3-dev libssl-dev libtidy-dev libtool libwrap0-dev libxml2-dev libxslt1-dev libyaml-dev libzip-dev libzmq3-dev make patch php-common snmp shtool systemd tzdata uuid-dev
...
if [[ "$VERSION_ID" != '24.04' && "$VERSION_ID" != '26.04' && "$arch" != "aarch64" && "$arch" != "arm64" ]]; then
