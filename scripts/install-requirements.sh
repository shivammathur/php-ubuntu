...
apt-get update
DEBIAN_FRONTEND=noninteractive apt-get install -y apache2 apt-fast automake gcc jq make pkg-config shtool libtool snmp sudo systemd unzip php"${PHP_VERSION:?}"-common php"${PHP_VERSION:?}"-imagick
